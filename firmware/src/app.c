    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_CHECK_CDC_READ:
            ...
            if(appData.isReadComplete == true)
            {
                ...
                int pixel,k=0,factor=4;
                float right_ratio, left_ratio;
                while (appData.readBuffer[k]){
                    message[k]=appData.readBuffer[k];
                    k++;
                }
                //OC1  left   OC2 : right
                pixel=atoi(message);
                if (pixel > 320){
                    left_ratio= factor*((float) pixel-320)/((float) 480-15);
                    OC1RS=1900+800*left_ratio;
                    if (OC1RS>2000)
                        OC1RS=2000;
                    right_ratio=factor*((float) pixel-320)/((float) 480-15);
                    OC2RS=1900-800*right_ratio;
                }
                else{
                    left_ratio= factor*((float) 320-pixel)/((float) 480-15);
                    OC1RS=1900-800*left_ratio;
                    right_ratio=factor*((float) 320-pixel)/((float) 480-15);
                    OC2RS=1900+800*right_ratio;
                    if (OC2RS>2000)
                        OC2RS=2000;
                }
